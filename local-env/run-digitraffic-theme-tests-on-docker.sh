...
# This script runs pytest tests for the ckanext-digitraffic_theme extension inside a Docker container.
...
#
# Arguments:
#   $1 (optional) - Test file name relative to the `tests` directory inside the ckanext-digitraffic_theme extension
...
# Examples:
#   Run all tests:
#   ./run-digitraffic-theme-tests-on-docker.sh
#   Run all tests in a specific file:
#   ./run-digitraffic-theme-tests-on-docker.sh auth/test_user_auth.py
#   Run a specific test function in a specific file:
#   ./run-digitraffic-theme-tests-on-docker.sh auth/test_user_auth.py TestUserAuth::test_user_show_self

set -euo pipefail

TESTS_DIRECTORY="ckanext/digitraffic_theme/tests"
TEST_FILE=
TEST_FUNCTION=
