...
try:
    ...
    vocab_all_labels = [
        list(g.objects(subject, SKOS.prefLabel)) for subject in vocab_subjects
    ]
    ...
    supported_languages_order = {"en": 0, None: 0, "fi": 1, "sv": 2}

    def sort_languages(labels):
        return sorted(
            labels,
            key=lambda label: supported_languages_order.get(label.language, None),
        )

    def is_supported_language(label):
        return str(label.language) in {
            str(key) for key in supported_languages_order.keys()
        }

    def contains_supported_language(labels):
        return (
            len(list(filter(lambda label: is_supported_language(label), labels))) != 0
        )

    all_used_labels = [
        sort_languages([label for label in labels if is_supported_language(label)])
        for labels in vocab_all_labels
    ]
    labels_without_supported_language = [
        labels for labels in vocab_all_labels if not contains_supported_language(labels)
    ]
    ...
    def add_empty_cells(labels):
        return [
            labels[index] if len(labels) > index else None
            for index in range(len(most_labels))
        ]
    ...
    with open(file_abspath, "w", newline="", encoding="utf-8") as csv_file:
        ...
        writer.writerow(
            [
                f'Vocabularies for {fetch_info.graph_url}\n\nGenerated using "rdf_to_csv.py {VOCAB}"'
            ]
        )
        writer.writerow([])
        writer.writerow(
            [
                str(label.language or "-- Language is not known --")
                for label in most_labels
            ]
        )
        writer.writerows(
            [
                add_empty_cells([str(label) for label in labels])
                for labels in all_used_labels
                if len(labels) > 0
            ]
        )
    if len(labels_without_supported_language) > 0:
        ...
        with open(
            unsupported_file_abspath, "w", newline="", encoding="utf-8"
        ) as csv_file:
            writer = csv.writer(csv_file, delimiter=";")
            writer.writerow(
                [
            ...
            for labels in labels_without_supported_language:
                writer.writerows(
                    [[str(label), str(label.language)] for label in labels]
                )
                writer.writerow(["---"])
except Exception:
