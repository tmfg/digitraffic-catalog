...
class ClassConverter:
    ...
    def convert(
        clazz: RDFSClass,
        ds: Dataset,
        omit: Dict[URIRef, Set[URIRef] | Literal["all"]] = {},
        ...
        for p in class_properties:
            ...
            if schema is None:
                continue
            is_range_value_class = isinstance(schema, Dict) and set(schema.keys()) == {
                "required"
            }
            if is_range_value_class:
                rdf_range = converter.get_range_value(ds, p)
                schema = ClassConverter.convert(
                    rdf_range, ds, omit=omit, is_required=schema.get("required")
                )
                append_schema(schema)
            else:
                append_schema(schema)
