ckan.module('digitraffic-theme-repeating-subfields', function($) {
  return {
    initialize: function() {
      $.proxyAll(this, /_on/);
    ...
    _findClosestDescendants: function(selector: string) {
      const thisEl = this.el
      return this.el.find(selector).filter(function(index) {
        return this.closest('[data-module="digitraffic-theme-repeating-subfields"]') === thisEl[0]
      })
    },
...
} as ckan.Module<HTMLFieldSetElement>);