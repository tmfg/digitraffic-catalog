...
def _is_user_accessing_own_data(
    user_triggering_action: User, acted_on_user: User
) -> bool:
    """
...
def _get_acting_and_acted_on_user(
...
def _is_user_action_allowed(
    context: Context, data_dict: DataDict, user_action: UserAction
) -> bool:
    try:
        user_triggering_action, acted_on_user = _get_acting_and_acted_on_user(
            context, data_dict
        )
        ...
        if user_action == UserAction.UPDATE:
            return _is_user_accessing_own_data(
                user_triggering_action, acted_on_user
            ) and _is_fixed_fields_unmodified(acted_on_user)
        return False
    except ValueError:
        return False
...
def user_show(next_auth: Callable, context: Context, data_dict: DataDict):
    is_user_action_allowed = _is_user_action_allowed(
        context, data_dict, UserAction.SHOW
    )
