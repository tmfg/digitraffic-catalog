...
class MobilityData:
    ...
    def __init__(self, dataset_dict: dict[str, Any], dataset_ref: URIRef):
        organization_ref = dataset_dict.get('publisher_uri', publisher_uri_organization_fallback(dataset_dict))
        dataset = Dataset(dataset_ref, {
            "description": Literal(dataset_dict["notes"]),
            "distribution": [Distribution(resource_uri(dist), dist) for dist in dataset_dict["resources"]],
