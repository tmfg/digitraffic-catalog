...
def fluent_core_translated_output(field, schema):
    assert field["field_name"].endswith(
        LANG_SUFFIX
        ...
        if value is not missing:
            ...
            for lang, text in value.items():
                ...
                if isinstance(text, str):
                    try:
                        value[lang] = text if six.PY3 else text.decode("utf-8")
                    except UnicodeDecodeError:
...
def fluent_tags(field, schema):
    ...
    if field and "tag_validators" in field:
        tag_validators = validators_from_string(field["tag_validators"], field, schema)
    ...
    def validator(key, data, errors, context):
        ...
        if value is not missing:
            ...
            for lang, keys in value.items():
                ...
                for i, v in enumerate(keys):
                    ...
                    if isinstance(v, str):
                        try:
                            out.append(v if six.PY3 else v.decode("utf-8"))
                        except UnicodeDecodeError:
                            errors[key].append(
                                _(
                                    "expected UTF-8 encoding for "
                                    '"{lang}" value item {num}'
                                ).format(lang=lang, num=i)
                            )
                    else:
                        out.append(v)
        ...
        for name, text in extras.items():
            ...
            if isinstance(text, str):
                try:
                    text = text if six.PY3 else text.decode("utf-8")
                except UnicodeDecodeError:
