...
class _SchemingMixin(object):
    ...
    def _load_presets(config):
        ...
        presets = reversed(config.get("scheming.presets", DEFAULT_PRESETS).split())
    ...
    def update_config(self, config):
        ...
        self._is_fallback = p.toolkit.asbool(config.get(self.FALLBACK_OPTION, False))
...
class SchemingDatasetsPlugin(p.SingletonPlugin, DefaultDatasetForm, _SchemingMixin):
    ...
    def validate(self, context, data_dict, schema, action):
        ...
        if not t or t not in self._schemas:
            return data_dict, {"type": ["Unsupported dataset type: {t}".format(t=t)]}
        ...
        def composite_convert_to(key, data, errors, context):
            ...
            for f in composite_convert_fields:
                if f not in unflat:
                    continue
                data[(f,)] = json.dumps(
                    unflat[f], default=lambda x: None if x == missing else x
                )
        ...
        if action_type == "show":
            ...
            if composite_convert_fields:
                schema = dict(
                    schema, __after=schema.get("__after", []) + [composite_convert_to]
                )
    ...
    def setup_template_variables(self, context, data_dict):
        super(SchemingDatasetsPlugin, self).setup_template_variables(context, data_dict)
        ...
        for fieldname_key in sorted(keys_from_data):
            ...
            for parts_idx, part in enumerate(parts_grouped):
                ...
                if idx_map := helpers.get_at_depth(indexes, stacked_keys):
                    marked_idx_map = idx_map.setdefault(
                        marked_index, {IDX_KEY: len(idx_map)}
                    )
                    idx = marked_idx_map[IDX_KEY]
                else:
                    idx = 0
                    helpers.set_at_depth(
                        indexes, stacked_keys, {marked_index: {IDX_KEY: idx}}
                    )
...
class SchemingGroupsPlugin(
    p.SingletonPlugin, _GroupOrganizationMixin, DefaultGroupForm, _SchemingMixin
):
    p.implements(p.IConfigurer)
    p.implements(p.ITemplateHelpers)
...
class SchemingOrganizationsPlugin(
    p.SingletonPlugin, _GroupOrganizationMixin, DefaultOrganizationForm, _SchemingMixin
):
    p.implements(p.IConfigurer)
    p.implements(p.ITemplateHelpers)
    p.implements(p.IGroupForm, inherit=True)
    ...
    def get_actions(self):
        return {
            "scheming_organization_schema_list": logic.scheming_organization_schema_list,
            "scheming_organization_schema_show": logic.scheming_organization_schema_show,
        }
...
def _field_output_validators(
    f, schema, convert_extras, convert_from_extras_type=convert_from_extras
):
    """
...
def _field_validators(f, schema, convert_extras):
    ...
    """
    if "validators" in f:
        validators = validation.validators_from_string(f["validators"], f, schema)
...
def _field_create_validators(f, schema, convert_extras):
    ...
    validators = validation.validators_from_string(f["create_validators"], f, schema)
...
def _expand_schemas(schemas):
    ...
    for name, original in schemas.items():
        ...
        for grouping in ("fields", "dataset_fields", "resource_fields"):
            ...
            schema[grouping] = [_expand(schema, field) for field in schema[grouping]]
